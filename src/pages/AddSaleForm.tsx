...
export default function AddSaleForm({ onClose }: { onClose: () => void }) {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            alert('Erreur lors de la validation de la vente : ' + (err.message || 'Une erreur inattendue est survenue'))
        } finally {
            setLoading(false)
        }
    }
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden animate-odoo-in">
                <div className="bg-indigo-900 px-8 py-6 text-white flex justify-between items-center">
                    <div className="flex items-baseline gap-2">
                        <h2 className="text-xl font-black text-white">Nouvelle Vente</h2>
                ...
                <div className="p-8 space-y-8 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center bg-slate-900 rounded-[2rem] p-8 text-white">
                        <div>
                            ...
                            <h3 className="text-3xl font-black text-white">{new Intl.NumberFormat('fr-FR').format(total)} <small className="text-sm opacity-50">FCFA</small></h3>
                        </div>
                        <div className="flex gap-4 mt-6 md:mt-0">
                            <button
                                onClick={handleSubmit}
                                disabled={loading}
                                className="bg-emerald-500 text-white px-10 py-4 rounded-2xl font-black shadow-lg shadow-emerald-900/20 hover:bg-emerald-600 disabled:opacity-50 transition-all uppercase tracking-widest text-[10px]"
                            >
